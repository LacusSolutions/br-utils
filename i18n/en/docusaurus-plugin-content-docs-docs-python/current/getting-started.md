...
- **`cpf`**: Access the CPF utilities (`CpfUtils`). Use `br_utils.cpf.format()`, `br_utils.cpf.generate()`, `br_utils.cpf.is_valid()` with the same options as in [cpf-utils](https://github.com/LacusSolutions/br-utils-py/blob/main/../cpf-utils/README.md).
- **`cnpj`**: Access the CNPJ utilities (`CnpjUtils`). Use `br_utils.cnpj.format()`, `br_utils.cnpj.generate()`, `br_utils.cnpj.is_valid()` with the same options as in [cnpj-utils](https://github.com/LacusSolutions/br-utils-py/blob/main/../cnpj-utils/README.md).
...
CPF methods are accessed via `utils.cpf`, `CpfUtils`, or the `cpf_*()` helpers from `br_utils.cpf`. CPF uses the v2 API from [`cpf-utils`](https://github.com/LacusSolutions/br-utils-py/blob/main/../cpf-utils/README.md): `str` or sequence input for `format()` / `is_valid()`, mapping or keyword overrides per call, and structured exceptions.
...
CNPJ methods are accessed via `utils.cnpj`, `CnpjUtils`, or the `cnpj_*()` helpers from `br_utils.cnpj`. CNPJ uses the v2 API from [`cnpj-utils`](https://github.com/LacusSolutions/br-utils-py/blob/main/../cnpj-utils/README.md).
...
**`br_utils.cpf`** — all exports from [cpf-utils](https://github.com/LacusSolutions/br-utils-py/blob/main/../cpf-utils/README.md) (e.g. `cpf_fmt`, `cpf_gen`, `cpf_val`, formatter/generator/validator classes, options, exceptions).

**`br_utils.cnpj`** — all exports from [cnpj-utils](https://github.com/LacusSolutions/br-utils-py/blob/main/../cnpj-utils/README.md) (e.g. `cnpj_fmt`, `cnpj_gen`, `cnpj_val`, formatter/generator/validator classes, options, exceptions).
...
|---------|----------------|--------|
| [`cpf-utils`](https://pypi.org/project/cpf-utils) | `CpfUtils`, `CpfFormatter`, `CpfGenerator`, `CpfValidator`, `cpf_fmt()`, `cpf_gen()`, `cpf_val()` | [docs](https://github.com/LacusSolutions/br-utils-py/blob/main/../cpf-utils/README.md) |
| [`cnpj-utils`](https://pypi.org/project/cnpj-utils) | `CnpjUtils`, `CnpjFormatter`, `CnpjGenerator`, `CnpjValidator`, `cnpj_fmt()`, `cnpj_gen()`, `cnpj_val()` | [docs](https://github.com/LacusSolutions/br-utils-py/blob/main/../cnpj-utils/README.md) |
...
See [CHANGELOG](https://github.com/LacusSolutions/br-utils-py/blob/main/CHANGELOG.md) for a list of changes and version history.
