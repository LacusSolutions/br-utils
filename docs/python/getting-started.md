...
- **`cpf`**: Acesso aos utils de CPF (`CpfUtils`). Use `br_utils.cpf.format()`, `br_utils.cpf.generate()`, `br_utils.cpf.is_valid()` com as mesmas opções do [cpf-utils](https://github.com/LacusSolutions/br-utils-py/blob/main/../cpf-utils/README.pt.md).
- **`cnpj`**: Acesso aos utils de CNPJ (`CnpjUtils`). Use `br_utils.cnpj.format()`, `br_utils.cnpj.generate()`, `br_utils.cnpj.is_valid()` com as mesmas opções do [cnpj-utils](https://github.com/LacusSolutions/br-utils-py/blob/main/../cnpj-utils/README.pt.md).
...
Os métodos de CPF são acessados via `utils.cpf`, `CpfUtils` ou os helpers `cpf_*()` de `br_utils.cpf`. O CPF usa a API v2 de [`cpf-utils`](https://github.com/LacusSolutions/br-utils-py/blob/main/../cpf-utils/README.pt.md): entrada `str` ou sequência em `format()` / `is_valid()`, sobrescritas por mapeamento ou palavra-chave por chamada e exceções estruturadas.
...
Os métodos de CNPJ são acessados via `utils.cnpj`, `CnpjUtils` ou os helpers `cnpj_*()` de `br_utils.cnpj`. O CNPJ usa a API v2 de [`cnpj-utils`](https://github.com/LacusSolutions/br-utils-py/blob/main/../cnpj-utils/README.pt.md).
...
**`br_utils.cpf`** — todas as exportações de [cpf-utils](https://github.com/LacusSolutions/br-utils-py/blob/main/../cpf-utils/README.pt.md) (ex.: `cpf_fmt`, `cpf_gen`, `cpf_val`, classes de formatador/gerador/validador, opções, exceções).

**`br_utils.cnpj`** — todas as exportações de [cnpj-utils](https://github.com/LacusSolutions/br-utils-py/blob/main/../cnpj-utils/README.pt.md) (ex.: `cnpj_fmt`, `cnpj_gen`, `cnpj_val`, classes de formatador/gerador/validador, opções, exceções).
...
|---------|----------------|--------|
| [`cpf-utils`](https://pypi.org/project/cpf-utils) | `CpfUtils`, `CpfFormatter`, `CpfGenerator`, `CpfValidator`, `cpf_fmt()`, `cpf_gen()`, `cpf_val()` | [docs](https://github.com/LacusSolutions/br-utils-py/blob/main/../cpf-utils/README.pt.md) |
| [`cnpj-utils`](https://pypi.org/project/cnpj-utils) | `CnpjUtils`, `CnpjFormatter`, `CnpjGenerator`, `CnpjValidator`, `cnpj_fmt()`, `cnpj_gen()`, `cnpj_val()` | [docs](https://github.com/LacusSolutions/br-utils-py/blob/main/../cnpj-utils/README.pt.md) |
...
Veja o [CHANGELOG](https://github.com/LacusSolutions/br-utils-py/blob/main/CHANGELOG.md) para alterações e histórico de versões.
